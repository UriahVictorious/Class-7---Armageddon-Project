    ...
    Statement = [
      {
        ...
        Resource = aws_secretsmanager_secret.bns-1c_db_secret05.arn
      },
      {
        Sid    = "EC2Describe"
        Effect = "Allow"
...
resource "aws_iam_policy" "bns-1c_leastpriv_read_params" {
  name        = "${local.bns-1c_prefix}_lp_ssm_read"
    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:ssm:${data.aws_region.bns-1c_region01.name}:${data.aws_caller_identity.bns-1c_self01.account_id}:parameter/lab/db/*"
        ]
      }
    ]
  })
...
resource "aws_iam_policy" "bns-1c_leastpriv_read_secret" {
  name        = "${local.bns-1c_prefix}_lp_secrets_read"
    ...
    Statement = [
      {
        ...
        Resource = local.bns-1c_secret_arn_guess
      }
    ]
  })
}
...
resource "aws_iam_policy" "bns-1c_leastpriv_cwlogs" {
  name        = "${local.bns-1c_prefix}_lp_cwlogs"
